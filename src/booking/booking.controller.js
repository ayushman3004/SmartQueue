...
export const book = asyncHandler(async (req, res) => {
  const { businessId, startTime, serviceType, notes, pricingLabel, couponCode, isGroupBooking, guestCount } = req.body;
...
export const extend = asyncHandler(async (req, res) => {
  ...
  res.json(new ApiResponse(200, result, `Extended by ${extraMinutes} min.`));
});
...
export const startService = asyncHandler(async (req, res) => {
  const booking = await bookingService.startService(req.params.id, req.user._id, req.user.role);
  const io = req.app.get("io");
  if (io) {
    io.to(`business:${booking.businessId}`).emit("service:started", {
...
export const completeService = asyncHandler(async (req, res) => {
  const booking = await bookingService.completeService(req.params.id, req.user._id, req.user.role);
  const io = req.app.get("io");
  if (io) {
    io.to(`business:${booking.businessId}`).emit("service:completed", {
      businessId: booking.businessId,
      bookingId: booking._id,
      booking,
    });
...
export const cancel = asyncHandler(async (req, res) => {
  const io = req.app.get("io");
  const result = await bookingService.cancelBooking(req.params.id, req.user._id, io);
  const message = result.refunded > 0
    ? `Booking cancelled. ₹${result.refunded} refunded to your wallet.`
    : "Booking cancelled.";
