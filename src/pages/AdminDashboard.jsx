...
const StatCard = ({ icon, label, value, color }) => (
  <div className="bg-white border border-zinc-200 p-8 rounded-[2rem] flex items-center justify-between group overflow-hidden relative shadow-lg hover:shadow-xl transition-all">
    ...
    <div className="space-y-2 relative z-10">
      ...
      <div className="text-4xl font-black text-zinc-950 tracking-tighter leading-none">{value}</div>
    </div>
...
export default function AdminDashboard() {
  ...
  const deleteHub = async (id) => {
    if (!window.confirm("CRITICAL: This action will permanently erase this node and all associated data. Proceed?")) return
    try {
      await axios.delete(`/admin/business/${id}`)
      toast.success("Hub node permanently purged from system")
      fetchStats()
    } catch (err) {
    ...
    <div className="container py-12 space-y-12 max-w-7xl px-4 sm:px-6">
      <div className="flex items-end justify-between border-b border-zinc-100 pb-8">
        <div className="space-y-3">
          ...
          <h1 className="text-5xl font-black tracking-tighter text-zinc-950 uppercase leading-none">Global <span className="text-teal-600">Control</span> Hub</h1>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-10">
        <div className="bg-white border border-zinc-200 p-10 rounded-[3rem] shadow-2xl space-y-10 relative overflow-hidden">
          ...
          <div className="flex items-center justify-between relative z-10">
            <h2 className="text-xl font-black uppercase tracking-tighter text-zinc-950">Recent Domain Assignments</h2>
          </div>
            ...
            {(data?.recent?.businesses || []).map(b => (
              <div key={b._id} className="p-5 rounded-3xl bg-slate-50 border border-zinc-100 flex items-center justify-between group hover:bg-white hover:border-zinc-200 transition-all shadow-xs">
                <div className="flex items-center gap-5">
                  ...
                  <div>
                    <h4 className="text-sm font-black text-zinc-950 transition-colors uppercase tracking-tight">{b.name}</h4>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button 
                    onClick={() => deleteHub(b._id)}
                    className="p-2.5 rounded-2xl bg-zinc-950 text-white hover:bg-rose-600 transition-colors shadow-lg"
                    title="Purge Node"
                  >
                    <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path strokeLinecap="round" strokeLinejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                  </button>
                </div>
              </div>
            ))}
            {(!data?.recent?.businesses?.length) && <p className="text-xs text-slate-400 font-bold uppercase tracking-widest py-8 text-center border border-dashed border-zinc-100 rounded-2xl">No recent hub nodes.</p>}
          </div>
        </div>

        <div className="bg-white border border-zinc-200 p-10 rounded-[3rem] shadow-2xl space-y-10 relative overflow-hidden">
          <div className="absolute top-0 right-0 w-64 h-64 bg-slate-50/50 rounded-full blur-[80px] pointer-events-none" />
          <h2 className="text-xl font-black uppercase tracking-tighter text-zinc-950 relative z-10">System Transaction Log</h2>
          <div className="space-y-4 relative z-10">
            {(data?.recent?.bookings || []).map(book => (
              <div key={book._id} className="p-5 rounded-3xl bg-slate-50 border border-zinc-100 flex items-center gap-5 hover:bg-white hover:border-zinc-200 transition-all shadow-xs group">
                ...
                <div>
                  <h4 className="text-sm font-black text-zinc-950 uppercase tracking-tight">Signal: New Node Entry</h4>
            ...
            {(!data?.recent?.bookings?.length) && <p className="text-xs text-slate-400 font-bold uppercase tracking-widest py-8 text-center border border-dashed border-zinc-100 rounded-2xl">Log is currently clear.</p>}
          </div>
        </div>
      </div>
    </div>
